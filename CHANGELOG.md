...
- **Proxy support** — `ClientOptions { proxy_url }` and `LinkedInClient::with_options`/`options()`. When set, API requests route through an HTTP(S)/SOCKS proxy. The proxy URL is read from `LINKEDIN_PROXY_URL`, then `HTTPS_PROXY`, then `HTTP_PROXY` (blank values ignored); existing constructors preserve behaviour via `ClientOptions::from_env()`.
- **Spam inbox** — `messages list --category primary|spam` reads a specific inbox category via the new `ConversationCategory` enum and `get_conversations_by_category`. `get_conversations` stays as a `PrimaryInbox` wrapper so existing callers are unchanged. The category is validated offline before any network call.
- **Engagement rate** — `feed stats` now reports engagement rate (interactions ÷ views, where interactions = likes + comments + shares) per post and across the set, and as `engagement_rate_pct` in `--json`. Rendered `n/a` (JSON `null`) when a post has zero views. Pure computation over the already-fetched posts; no extra API call.
...
- **InMail replies fork a new thread** — `messages reply` (and `reply_to_conversation`) used to send `hostRecipientUrns` and rely on LinkedIn routing the reply to the matching recipient set. That works for regular member-to-member threads but silently created a *new* conversation for InMail/recruiter threads, while the CLI still printed a success receipt. The reply now pins the thread with `message.conversationUrn` (`urn:li:msg_conversation:(<mailbox>,<threadId>)`) and sends no `hostRecipientUrns`, matching the LinkedIn web client — captured from live traffic and verified end-to-end from the CLI (the reply lands in the same thread, no fork). A response-side check reports a thread mismatch as an error instead of a false success.
