...
The client falls back to `HTTPS_PROXY` or `HTTP_PROXY` if `LINKEDIN_PROXY_URL` is unset. For authenticated browser sessions, place a cookie map at `secrets/browser_cookies.json` with at least `li_at` and `JSESSIONID`; this is useful when importing cookies from a managed browser profile. `auth status` uses this browser-cookie fallback for the live API check.
