...
pub(super) fn save_feed_cache(value: &Value) -> CliResult<()> {
    ...
    let json = serde_json::to_string(value)
        .map_err(|e| CliError::Other(format!("failed to serialize feed cache: {e}")))?;
    std::fs::write(&path, json)
        .map_err(|e| CliError::Other(format!("failed to write feed cache: {e}")))?;
    Ok(())
}

pub(super) fn load_feed_cache() -> CliResult<Value> {
    let path = feed_cache_path()?;
    let data = std::fs::read_to_string(&path)
        .map_err(|_| "no cached feed. Run `feed list` or `feed my-posts` first.".to_string())?;
    serde_json::from_str(&data)
        .map_err(|e| CliError::Other(format!("failed to parse feed cache: {e}")))
}

/// Load a 1-based feed-cache element by index. Used by reactions + comments.
pub(super) fn cached_feed_element(index: usize) -> CliResult<Value> {
    if index == 0 {
    ...
    let elements = cache
        ...
        .ok_or_else(|| CliError::Other("cached feed has no elements array".to_string()))?;
