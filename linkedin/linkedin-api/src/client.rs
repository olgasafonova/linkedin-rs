...
impl LinkedInClient {
    ...
    /// Threshold in bytes above which GIF uploads are likely to fail on
    /// LinkedIn's image CDN sync endpoint (`/dms-uploads/sp/sync/v2/`), which
    /// has a ~10 s gateway timeout. From most servers anything above ~3 MB
    /// exceeds that window and the CDN returns 504.
    const GIF_SIZE_WARNING_THRESHOLD: usize = 3_000_000;

    /// Upload an image/GIF/video/PDF to LinkedIn media storage and return upload metadata.
    ///
    /// Handles single-part and multi-part uploads.
    ///
    /// **Large GIF warning:** GIF files larger than [`GIF_SIZE_WARNING_THRESHOLD`]
    /// will likely fail because LinkedIn's image CDN sync endpoint times out.
    /// Callers should check the file size before calling this method and warn
    /// the user. The recommended options are:
    /// 1. Re-render the GIF at lower quality/fewer frames to get under the limit
    /// 2. Convert to MP4 and upload as video (but note: videos don't autoplay)
    ///
    /// To explicitly upload a GIF as video, convert it to MP4 first and pass
    /// the `.mp4` file instead.
    pub async fn upload_media(&self, media_path: &Path) -> Result<Value, Error> {
        ...
        // Warn about large GIFs that will likely fail on the CDN.
        if upload_type == "IMAGE_SHARING" && content_type == "image/gif"
            && bytes.len() > Self::GIF_SIZE_WARNING_THRESHOLD
        {
            return Err(Error::InvalidInput(format!(
                "GIF is {:.1} MB — exceeds LinkedIn's CDN sync timeout limit (~3 MB). \
                 The upload will likely fail with HTTP 504.\n\n\
                 Options:\n  \
                 1. Re-render the GIF at lower quality or fewer frames to get under 3 MB\n  \
                 2. Convert to MP4 and upload as video (note: videos don't autoplay in feed)\n\n\
                 To upload as video, convert the file to .mp4 first and pass the .mp4 path instead.",
